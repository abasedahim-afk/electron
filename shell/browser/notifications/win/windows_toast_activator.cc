...
void EnsureShortcut() {
  ...
  // If a shortcut already exists, try to inherit its CLSID so we keep a
  // stable activator across launches (activation breaks if the CLSID changes
  // between when the toast was shown and when it is clicked).
  if (base::PathExists(lnk_path)) {
    Microsoft::WRL::ComPtr<IShellLink> existing;
      ...
      if (SUCCEEDED(existing.As(&pf)) &&
        ...
        if (SUCCEEDED(existing.As(&store))) {
          ...
          if (SUCCEEDED(store->GetValue(PKEY_AppUserModel_ToastActivatorCLSID,
            ...
            // Normalize and set the global CLSID if user hasn't explicitly
            // set one yet (setter validates/normalizes formatting).
            wchar_t buf[64] = {0};
...
void NotificationActivator::RegisterActivator() {
  if (g_registered_)
    return;
  // Order matters: ensure (and possibly inherit) shortcut CLSID before
  // writing registry so that we don't generate a new CLSID that mismatches
  // an existing shortcut (which would break activation callbacks).
  EnsureShortcut();
  EnsureCLSIDRegistry();

  CLSID clsid;
  if (FAILED(::CLSIDFromString(GetAppToastActivatorCLSID(), &clsid))) {
    LOG(ERROR) << "Invalid toast activator CLSID";
    return;
  }

  if (!g_factory_instance)
    g_factory_instance = new (std::nothrow) NotificationActivatorFactory();
  if (!g_factory_instance)
    return;
